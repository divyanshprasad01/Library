...
//    method which takes url and does all the work and returns the list of earthquakes with required information to show on main ui.
    public static List<BookObj> fetch(String Surl){
...
//    Extracts required fields from the json response, and returns the list of events.
    private static List<BookObj> extractFeaturesfromJSON(String output){
        if(output.length()==0){
            return null;
        }

       List<BookObj> Events = new ArrayList<>();
        BookObj Event ;

        try {
            JSONObject baseObj = new JSONObject(output);

            JSONArray feturesArray = baseObj.getJSONArray("items");


            for(int i=0; i<feturesArray.length(); i++){

                JSONObject BooksJSONObj = feturesArray.getJSONObject(i);
                JSONObject values = BooksJSONObj.getJSONObject("volumeInfo");
